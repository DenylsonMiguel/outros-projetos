...
const despertador: (day: number, vacations: boolean) => string = (
	day,
	vacations
) => {
	const isWeekened: boolean = day === 0 || day === 6;
	if (!isWeekened && !vacations) return "7:00";
	else if (isWeekened && !vacations) return "10:00";
	else if (vacations && !isWeekened) return "10:00";
	return "off";
};